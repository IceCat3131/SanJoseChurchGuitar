...
window.__chordTimeline = [];
window.__autoAccompanimentSong = [];
window.__autoAccompanimentTabTrack = [];
...
function mapHarmonyKindToSuffix(kindEl) {
if (!kindEl)
return '';
const textAttr = String(kindEl.getAttribute('text') || '').trim();
if (textAttr)
return textAttr;
const v = String(kindEl.textContent || '').trim();
const map = {
...
'suspended-second': 'sus2', 'suspended-fourth': 'sus4', 'power': '5', 'none': ''
};
return Object.prototype.hasOwnProperty.call(map, v) ? map[v] : v;
}
function buildChordSymbolFromHarmony(harmonyEl) {
if (!harmonyEl)
return '';
const rootStep = String(harmonyEl.querySelector('root > root-step')?.textContent || '').trim();
if (!rootStep)
return '';
const rootAlter = String(harmonyEl.querySelector('root > root-alter')?.textContent || '').trim();
const bassStep = String(harmonyEl.querySelector('bass > bass-step')?.textContent || '').trim();
const bassAlter = String(harmonyEl.querySelector('bass > bass-alter')?.textContent || '').trim();
...
const suffix = mapHarmonyKindToSuffix(kindEl);
return root + suffix + (bass ? ('/' + bass) : '');
}
function getTimeSignatureText(beats, beatType) {
if (!Number.isFinite(beats) || !Number.isFinite(beatType) || !beats || !beatType)
return '--';
return String(beats) + '/' + String(beatType);
}
function buildMusicTimelineFromXML(xmlText) {
const parser = new DOMParser();
const xml = parser.parseFromString(xmlText, 'application/xml');
const part = xml.querySelector('score-partwise > part, score-timewise > part');
if (!part)
throw new Error('未找到 MusicXML part');
let divisions = 1, beats = 4, beatType = 4, currentStartTicks = 0;
const timeSignatures = [], chords = [], measures = [];
part.querySelectorAll(':scope > measure').forEach((measureEl, idx) => {
const measureNo = parseInt(measureEl.getAttribute('number') || '', 10) || (idx + 1);
...
const beatsEl = measureEl.querySelector(':scope > attributes > time > beats');
const beatTypeEl = measureEl.querySelector(':scope > attributes > time > beat-type');
if (beatsEl && beatTypeEl) {
const nb = parseInt(beatsEl.textContent || '4', 10);
const nt = parseInt(beatTypeEl.textContent || '4', 10);
if (Number.isFinite(nb) && nb > 0)
beats = nb;
if (Number.isFinite(nt) && nt > 0)
beatType = nt;
}
const meterText = getTimeSignatureText(beats, beatType);
timeSignatures.push({ measure: measureNo, meter: meterText, divisions });
let pos = 0, maxPos = 0;
Array.from(measureEl.children).forEach((node) => {
const name = node.nodeName;
if (name === 'note') {
const dur = parseInt(node.querySelector('duration')?.textContent || '0', 10) || 0;
pos += dur;
if (pos > maxPos)
maxPos = pos;
return;
...
if (name === 'harmony') {
const offset = parseInt(node.querySelector('offset')?.textContent || '0', 10) || 0;
const tick = Math.max(0, pos + offset);
const chordRaw = buildChordSymbolFromHarmony(node);
if (chordRaw)
chords.push({ measure: measureNo, beat: tick / divisions, tickFromScoreStart: currentStartTicks + tick, tickInMeasure: tick, divisions, meter: meterText, chordRaw });
}
});
let nominal = 0;
if (Number.isFinite(beats) && Number.isFinite(beatType) && beatType > 0)
nominal = Math.round((beats * 4 / beatType) * divisions);
const durationTicks = Math.max(maxPos, nominal, 1);
measures.push({ measure: measureNo, startTicks: currentStartTicks, durationTicks, endTicks: currentStartTicks + durationTicks, divisions, beats, beatType, meter: meterText });
currentStartTicks += durationTicks;
});
return { chords, measures, timeSignatures, totalTicks: currentStartTicks };
}
function remapTimelineTimesIfNeeded() {
const raw = window.__musicTimelineRaw;
const totalMs = Math.max(0, window.__playbackState?.total || 0);
if (!raw || !raw.totalTicks || !totalMs)
return;
if (raw.__mappedTotalMs === totalMs)
return;
const tickToMs = (ticks) => Math.round((ticks / raw.totalTicks) * totalMs);
window.__measureTimelineRaw = (raw.measures || []).map(m => ({ ...m, startMs: tickToMs(m.startTicks), endMs: tickToMs(m.endTicks), durationMs: Math.max(1, tickToMs(m.endTicks) - tickToMs(m.startTicks)) }));
window.__chordTimelineRaw = (raw.chords || []).map(c => ({ ...c, timeMs: tickToMs(c.tickFromScoreStart) }));
raw.__mappedTotalMs = totalMs;
}
const BOOKS = {
c: {
label: "大本诗歌",
...
if (!name || !Number.isFinite(semitones))
return name || '--';
...
if (!raw || !Number.isFinite(semitones) || semitones === 0)
return raw;
...
const quality = m[2] || '';
const bass = m[3] || '';
if (NOTE_TO_INDEX[root] == null)
return raw;
if (bass && NOTE_TO_INDEX[bass] == null)
return raw;
...
if (!/^[A-G](?:#|b)?/.test(s))
return false;
if (/[^A-Ga-g0-9#b\/()\-+susmajdimaugaddnoø°△mM ]/.test(s))
return false;
...
if (!looksLikeChordSymbol(original))
return;
...
try {
if (__chordRewriteTimer)
clearTimeout(__chordRewriteTimer);
}
catch (e) { }
...
const semitones = getUserTranspose();
if (!original || original === '--' || semitones === 0)
return original;
...
if (!tracks.length)
return [];
if (Array.isArray(__visibleTrackIndices))
...
return;
const tracks = getAllTracks(api);
if (!tracks.length || typeof api.changeTrackMute !== 'function')
return;
...
try {
if (!api || api.__finalPlayerGuardsInstalled)
return;
...
if (currentInput)
state.searchDraft = currentInput;
syncBookButtons();
clearSearchResults();
syncUI();
...
return Math.max(0, Math.min(11, cached));
...
return Math.max(0, Math.min(11, c));
try {
...
return Math.max(0, Math.min(11, a));
}
}
catch (e) { }
return 0;
...
if (!note || !Number.isFinite(note.fret) || note.fret < 0)
return null;
...
if ((!offset && !isAuto) || !lookup || !Array.isArray(lookup.staffSystems))
return;
...
try {
if (__tabCapoOverlayTimer)
clearTimeout(__tabCapoOverlayTimer);
}
catch (e) { }
...
try {
if (__autoTabRewriteTimer)
clearTimeout(__autoTabRewriteTimer);
}
catch (e) { }
__autoTabRewriteTimer = setTimeout(() => {
try {
if (typeof applyCurrentPatternToOriginalTab === 'function')
applyCurrentPatternToOriginalTab();
}
catch (e) {
console.warn('applyCurrentPatternToOriginalTab failed', e);
}
}, delay);
}
...
if (!res.ok)
throw new Error(`HTTP ${res.status} ${gtzUrl}`);
...
else if (gtz?.score?.data)
scoreB64 = gtz.score.data;
else if (gtz?.scoreBase64)
scoreB64 = gtz.scoreBase64;
if (!scoreB64)
...
if (!settings.display.resources.elementFonts)
...
if (!fontObj)
return;
if (ctx.ne.EffectLyrics != null)
ctx.settings.display.resources.elementFonts.set(ctx.ne.EffectLyrics, fontObj);
api.updateSettings();
}
catch (e) {
...
if (!fontObj)
return;
if (ctx.ne.EffectChordNames != null)
ctx.settings.display.resources.elementFonts.set(ctx.ne.EffectChordNames, fontObj);
api.updateSettings();
}
catch (e) {
...
if (width < 520)
return isLandscape ? 0.88 : 0.92;
if (width < 740)
return isLandscape ? 0.90 : 0.94;
if (width < 1100)
return 0.96;
return 1.0;
}
...
if (!staff)
continue;
if (staff.__origShowStandardNotation == null)
staff.__origShowStandardNotation = !!staff.showStandardNotation;
if (staff.__origShowTablature == null)
staff.__origShowTablature = !!staff.showTablature;
if ('showNumbered' in staff && staff.__origShowNumbered == null)
staff.__origShowNumbered = !!staff.showNumbered;
if ('showNumberedNotation' in staff && staff.__origShowNumberedNotation == null)
staff.__origShowNumberedNotation = !!staff.showNumberedNotation;
}
}
}
...
let staffMode = 'five';
try {
staffMode = localStorage.getItem('staffMode') || 'five';
}
catch (e) { }
...
if (keep) {
if (staff.__origShowStandardNotation != null)
staff.showStandardNotation = !!staff.__origShowStandardNotation;
if (staff.__origShowTablature != null)
staff.showTablature = !!staff.__origShowTablature;
if ('showNumbered' in staff && staff.__origShowNumbered != null)
staff.showNumbered = !!staff.__origShowNumbered;
if ('showNumberedNotation' in staff && staff.__origShowNumberedNotation != null)
staff.showNumberedNotation = !!staff.__origShowNumberedNotation;
if (!staff.showTablature) {
staff.showStandardNotation = (staffMode !== 'jian');
if ('showNumbered' in staff)
staff.showNumbered = (staffMode === 'jian');
if ('showNumberedNotation' in staff)
staff.showNumberedNotation = (staffMode === 'jian');
}
}
else {
...
if ('showNumberedNotation' in staff)
staff.showNumberedNotation = false;
}
});
});
}
function renderVisibleTracks(api) {
if (!api || !__currentScore)
return;
...
if (api.score && api.score !== __currentScore)
applyTrackSelectionToScore(api.score, idxs);
...
if (!settings.player)
settings.player = {};
if ('enableCursor' in settings.player)
settings.player.enableCursor = true;
if ('enableAnimatedBeatCursor' in settings.player)
...
const progressEl = document.getElementById("ui-progress");
if (timeEl)
timeEl.textContent = `${msToMMSS(current)} / ${msToMMSS(total)}`;
if (progressEl)
progressEl.value = "0";
updatePlayButtonVisual(!!playing);
}
...
if (AUTO_CHORD_SHAPES[c])
return { key: c, shape: AUTO_CHORD_SHAPES[c].slice() };
const slash = c.split('/')[0];
if (AUTO_CHORD_SHAPES[slash])
return { key: slash, shape: AUTO_CHORD_SHAPES[slash].slice() };
}
return null;
}
...
if (role === 'bass')
return b;
if (hint && hint >= 1 && hint <= 6 && isPlayableString(hint))
return hint;
...
while (s >= 1) {
const fret = shape[6 - s];
if (fret !== 'x' && fret !== 'X' && fret != null)
break;
s -= 1;
}
if (s < 1) {
for (let probe = 6; probe >= 1; probe--) {
if (isPlayableString(probe))
return probe;
}
s = 1;
}
...
if (playable.length && role !== 'bass')
return Array.from(new Set(playable));
...
try {
if (typeof window.getAutoDisplayedChordForOriginal === 'function')
chordDisplay = window.getAutoDisplayedChordForOriginal(c.chordRaw);
else if (typeof transposeChordSymbol === 'function')
chordDisplay = transposeChordSymbol(c.chordRaw, getUserTranspose(), prefersFlatsFromKeyName((window.__currentMeta && window.__currentMeta.key_name) || ''));
}
catch (e) { }
...
if (cm < mNo || (cm === mNo && cb <= atBeat + 1e-6))
chosen = c;
});
return chosen;
}
...
const starts = [];
if (seed)
starts.push({ beat: 0, chordObj: seed });
local.forEach((c) => {
if (c.beat < 0 || c.beat >= beatsInMeasure + 1e-6)
return;
const existing = starts.find((s) => Math.abs(s.beat - c.beat) < 1e-6);
if (existing)
existing.chordObj = c;
else
starts.push({ beat: c.beat, chordObj: c });
});
...
if (lastSeg && lastSeg.chordObj)
lastChordObj = lastSeg.chordObj;
return out;
}
...
if (localBeat >= segment.durationBeat - 1e-6)
return;
...
const quarterUnits = Number(divisions) || 1;
const ratio = durationUnits / quarterUnits;
if (meta && meta.tuplet) {
if (Math.abs(ratio - (1 / 3)) < 0.05 || ratio <= 0.5)
return 'eighth';
}
...
return 'eighth';
if (Math.abs(ratio - 0.375) < 0.05)
return '16th';
if (Math.abs(ratio - 0.25) < 1e-6)
return '16th';
if (Math.abs(ratio - 0.125) < 0.05)
return '32nd';
...
if (pitch.alter != null)
pitchEl.appendChild(createTextEl(doc, 'alter', pitch.alter));
...
technical.appendChild(createTextEl(doc, 'string', n.string));
technical.appendChild(createTextEl(doc, 'fret', playFret));
...
const divEl = measureEl.querySelector(':scope > attributes > divisions');
if (divEl)
divEl.textContent = String(divisions);
else {
...
while (measureEl.firstChild)
measureEl.removeChild(measureEl.firstChild);
keep.forEach(node => measureEl.appendChild(cloneXmlNode(xmlDoc, node)));
if (!autoBar || !Array.isArray(autoBar.notes) || !autoBar.notes.length)
continue;
...
g.beamStates = [];
if (g.tuplet)
g.tupletInfo = { kind: idx === 0 ? 'start' : (idx === cluster.length - 1 ? 'stop' : 'mid') };
else
g.tupletInfo = null;
});
...
if (!contiguous || !sameTuplet || breakForDotted)
flush();
cluster.push(g);
});
flush();
...
if (!window.__atApi || !window.__originalXmlText)
return;
if (__autoTabRewriteInProgress) {
__autoTabRewritePending = true;
return;
}
const scheme = (typeof window.getCurrentSchemeResult === 'function') ? window.getCurrentSchemeResult() : null;
if (!scheme || scheme.mode !== 'auto') {
...
try {
loadXmlTextIntoAlphaTab(window.__atApi, window.__originalXmlText);
}
finally {
setTimeout(() => { try {
scheduleTabCapoOverlay(140);
}
catch (e) { } __autoTabRewriteInProgress = false; if (__autoTabRewritePending) {
__autoTabRewritePending = false;
applyCurrentPatternToOriginalTab();
} }, 120);
}
}
...
if (window.__lastRenderedAutoXml === xmlText)
return;
window.__lastRenderedAutoXml = xmlText;
__autoTabRewriteInProgress = true;
try {
loadXmlTextIntoAlphaTab(window.__atApi, xmlText);
}
finally {
setTimeout(() => { try {
scheduleTabCapoOverlay(140);
...
try {
if (typeof window.applyStaffMode === 'function')
window.applyStaffMode({ domOnly: true });
}
catch (e) { }
});
...
const gtzUrl = `${gtzPrefix}${pad4(no)}_cn_g.gtz`;
...
const bookKey = state.currentBook === 'ts' ? 'ts' : 'c';
const prefix = bookKey === 'ts' ? 'TS' : 'C';
...
if (el)
el.textContent = getCurrentSongCodeLabel();
}
...
if (bigTitleEl)
bigTitleEl.textContent = bigTitle;
if (subTitleEl)
...
continue;
for (const staff of track.staves) {
if (!staff || staff.showTablature)
continue;
staff.showStandardNotation = (mode !== 'jian');
if ('showNumbered' in staff)
staff.showNumbered = (mode === 'jian');
if ('showNumberedNotation' in staff)
staff.showNumberedNotation = (mode === 'jian');
}
}
}
...
clearInterval(t);
try {
window.__atApi.renderFinished.on(() => setTimeout(() => fixRenderedJianpuDom(getStaffMode()), 30));
}
catch (e) { }
}
}, 300);
...
if ($('ui-key'))
$('ui-key').textContent = keyMeta || '--';
if ($('ui-capo'))
$('ui-capo').textContent = 'CP' + capoMeta;
if ($('ui-time'))
$('ui-time').textContent = currentTime;
if ($('ui-progress'))
$('ui-progress').value = String(__playbackState.progress || 0);
if ($('ui-speed-range'))
$('ui-speed-range').value = String(speedPct);
if ($('ui-speed-value'))
$('ui-speed-value').textContent = speedPct + '%';
if ($('ui-key-value'))
$('ui-key-value').textContent = transText;
if ($('ui-font-value'))
$('ui-font-value').textContent = fontText;
if ($('ui-spacing-value'))
$('ui-spacing-value').textContent = spacingText;
if ($('ui-chord-value'))
$('ui-chord-value').textContent = chordText;
const searchPanelOpen = $('panel-search') && !$('panel-search').hidden;
if ($('ui-search-input') && document.activeElement !== $('ui-search-input') && !searchPanelOpen)
$('ui-search-input').value = String(state.searchDraft || state.currentNo || '');
if ($('search-input') && document.activeElement !== $('search-input'))
$('search-input').value = String(state.searchDraft || state.currentNo || '');
...
const mode = (state.mode === 'lyrics') ? 'lyrics' : ((function () { try {
return localStorage.getItem('staffMode') || 'five';
}
catch (e) {
return 'five';
...
__visibleTrackIndices = sorted;
}
else {
window.__visibleTrackIndices = sorted;
}
}
catch (e) {
window.__visibleTrackIndices = sorted;
}
window.__visibleTrackIndices = sorted.slice();
}
function applyTrackSelection() {
try {
if (__trackSelectionRenderTimer)
clearTimeout(__trackSelectionRenderTimer);
...
console.warn('applyTrackSelection failed', err);
}
}, 10);
}
catch (e) {
...
return;
if (e.target.closest('#new-track-panel'))
return;
if (e.target.closest('#ui-track'))
return;
closeNewTrackPanel();
});
});
