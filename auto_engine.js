...
if (window.SchemeEngine && typeof SchemeEngine.isFlatKeyName === 'function')
return SchemeEngine.isFlatKeyName(keyName);
if (typeof prefersFlatsFromKeyName === 'function')
return !!prefersFlatsFromKeyName(keyName);
...
if (!chord)
return;
if (typeof looksLikeChordSymbol === 'function' && !looksLikeChordSymbol(chord))
return;
if (!seen.has(chord)) {
seen.add(chord);
out.push(chord);
...
if (!raw)
return;
if (typeof looksLikeChordSymbol === 'function' && !looksLikeChordSymbol(raw))
return;
if (!el.getAttribute('data-orig-chord'))
el.setAttribute('data-orig-chord', raw);
if (!seen.has(raw)) {
seen.add(raw);
out.push(raw);
}
...
if (out.length)
AUTO.cachedOriginalChords = out.slice();
...
if (!force && sig === __lastStateDispatchSignature)
return;
__lastStateDispatchSignature = sig;
setTimeout(() => {
try {
window.__autoTabDisplayCapo = getCurrentSchemeResult().activeCapo;
}
catch (e) { }
...
try {
return window.SchemeEngine ? SchemeEngine.shiftChord(ch, semis, useFlats) : ch;
}
catch (e) {
return ch;
}
...
updateUi();
if (typeof scheduleChordRewrite === 'function')
scheduleChordRewrite(20);
}
...
const note = $('ui-rhythm-note');
if (img)
img.src = inAuto ? 'assets/ui_icons/auto-accompany.svg' : 'assets/ui_icons/original.svg';
if (note)
note.textContent = inAuto ? '伴奏' : '原谱';
btn.setAttribute('aria-label', inAuto ? '自动伴奏' : '原谱');
}
['a', 'b', 'c'].forEach((k, idx) => {
...
if (typeof looksLikeChordSymbol === 'function' && !looksLikeChordSymbol(original))
return;
if (!el.getAttribute('data-orig-chord'))
...
if (AUTO.mode === 'auto') {
refreshSchemes();
if (!AUTO.currentPatternId)
AUTO.currentPatternId = AUTO.lastAutoPatternId || (window.RhythmEngine ? (RhythmEngine.getDefaultPattern(currentMeter()) || {}).id : null) || null;
}
applyScheme();
}
...
if (!list || !window.RhythmEngine)
return;
const patterns = RhythmEngine.getPatternsForMeter(currentMeter());
if (patterns.length && !AUTO.currentPatternId)
AUTO.currentPatternId = AUTO.lastAutoPatternId || patterns[0].id;
list.innerHTML = '';
...
if (AUTO.mode === 'auto' && AUTO.manualCapoMode)
return AUTO.manualCapo;
if (AUTO.mode === 'auto' && AUTO.currentScheme)
return AUTO.currentScheme.capo;
...
$('ui-manual-capo-minus')?.addEventListener('click', () => { if (AUTO.mode !== 'auto')
setMode('auto'); enterManualCapoFromCurrentSong(); setManualCapo(-1); });
$('ui-manual-capo-plus')?.addEventListener('click', () => { if (AUTO.mode !== 'auto')
setMode('auto'); enterManualCapoFromCurrentSong(); setManualCapo(1); });
}
function bootstrap() {
ensureUi();
